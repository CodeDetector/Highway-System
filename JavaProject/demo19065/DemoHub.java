...
public class DemoHub extends Hub {
	...
	@Override 
	protected synchronized void processQ(int detlaT) {
	...
	private synchronized int currentCapacity() {
		return this.truckQ.size();
	}


	// Routing functions
	public synchronized void route(Hub source, Hub dest) {
		phi.put(source,null);
		dfs(source);
		trace(dest);
	}
	...
	private synchronized void trace(Hub dest) {
		path.add(dest);
	...
	private synchronized void dfs(Hub hub) {
		visited.add(hub);
	...
	private synchronized void traverse(Hub currentHub, Location loc) {
	...
	private synchronized void refreshRoutingFields() {
		path.clear();
		visited.clear();
		phi.clear();
		traversed.clear();
